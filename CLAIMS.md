# CLAIMS.md — Verification Log

> Every factual claim in your Curator Cards should be verifiable. Log at least one claim per card here.
> This is the habit that separates professionals from prompt-paste operators.

---

## Card 1: Bauhaus

### Claim 1
- **Claim:** The school moved to Dessau and later Berlin before its closure, leaving a legacy of functionalist design that integrated art into every aspect of modern architecture.
- **Source link(s):** https://whc.unesco.org/en/list/729/
- **Confidence:** High
- **Notes:** UNESCO showcases different photos of the Bauhaus at their different locations. The Bauhaus' buildings are recognized as European modern architecture, embodying the different principles the school follow through.

---

## Card 2: Swiss Style

### Claim 1
- **Claim:** Originating in Switzerland and Germany, the Swiss Style—also known as the International Typographic Style—refined the principles of the Bauhaus into a rational and mathematical design language. 
- **Source link(s):** https://medium.com/fgd1-the-archive/beethoven-poster-by-josef-muller-brockmann-ce06940edf74
- **Confidence:** High
- **Notes:** Influences of the Bauhaus movement can be seen in Josef Müller-Brockmann's "Beethoven" poster, who was a leading designer in Swiss graphic design. The poster utilizes simple geometic shapes and clean sans-serif typography often found in the Bauhaus' works to convey musical rhythm. 

---

## Card 3: New Wave Typography

### Claim 1
- **Claim:** Emerged from the Basel School of Design in Switzerland, New Wave typography developed as a Postmodernism reaction against the rigid constraints of the Swiss Style
- **Source link(s):** https://collections.vam.ac.uk/item/O1155283/kunsthalle-basel-kunstkredit-76-77-poster-weingart-wolfgang/
- **Confidence:** High
- **Notes:** Weingart's "Kunsthalle Basel Kunstkredit 76-77" showchases the applied chaotic and stylistic elements of the New Wave typography that he experimented during his studies and teaching at the Basel School of Design.

---

## Reflection

**One AI claim I didn't trust at first and how I verified it:**

*[Write 2–3 sentences. What did the AI say? Why did it feel wrong? What source confirmed or denied it?]*

Gemini AI initially stated that the Bauhaus operated at three locations simultaneously. I recalled learning that the school was first established in Weimar, but there was no mention of any other buildings. I checked the article by UNESCO, which explained that the school relocated to Dessau and then Berlin after the first building closed in Weimar, so I corrected and rewrote the sentence from the AI to convey the actual statement.
